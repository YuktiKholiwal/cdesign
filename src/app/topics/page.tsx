...
export default async function TopicsPage() {
  ...
  return (
    <main className="mx-auto max-w-5xl px-5 py-16">
      ...
      {topics.length === 0 ? (
        <p className="mt-8 text-sm text-neutral-500 dark:text-neutral-400">No topics yet.</p>
      ) : (
        <div className="mt-10 flex flex-wrap gap-3">
          {topics.map(({ topic, count }) => (
            <Link
              ...
              className="card inline-flex items-center gap-2 px-4 py-2.5 text-sm font-medium text-neutral-700 dark:text-neutral-200 transition-all hover:-translate-y-0.5"
            >
              <span className="font-mono text-neutral-400 dark:text-neutral-500">#</span>
              {topic}
              <span className="rounded-full bg-neutral-100 dark:bg-neutral-800 px-1.5 py-0.5 font-mono text-xs text-neutral-500 dark:text-neutral-400">
                {count}
              </span>
            </Link>
          ))}
